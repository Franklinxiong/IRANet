...
pred_dir = ""
gt_dir = ""
method_name = 'glass_depth'

for sub_dir in os.listdir(pred_dir):
    if 0 == len(os.listdir(os.path.join(pred_dir, sub_dir, method_name))):
        continue
    iou_l = []
    acc_l = []
    mae_l = []
    ...
    for name in os.listdir(os.path.join(pred_dir, sub_dir, method_name)):
        gt = get_gt_mask(name, gt_dir)
        normalized_pred = get_normalized_predict_mask(name, os.path.join(pred_dir, sub_dir, method_name))
        binary_pred = get_binary_predict_mask(name, os.path.join(pred_dir, sub_dir, method_name))
